...
Plugin URI: http://www.alvinet.com/webmasters/widget.html
...
Author: iCode
Version: 1.1
Author URI: http://www.icodeitforyou.fr
*/


$instance = array("","","","","","","");

add_action('widgets_init', 'alvinet_init');

function alvinet_init(){
...
class alvinet_widget extends WP_widget{
    ...
    function widget($args, $instance){
		...
		<script>
			...
			function sendClickTrackToAlvinet(guid){
				(new Image()).src="http://www.alvinet.com/clktrk/id-"+guid; 
			};
        ...
        echo $before_widget;
       	...
       	echo $before_title.$instance['titre'].$after_title;
				...
				$rss = new DOMDocument();
				$rssUrl = 'http://alvinet.com/api/?nbr='.$limit.
						'&keyword='.$keyword.
						'&akeyword='.$akeyword.
						'&catid='.$catid.
				...
				$htmlUrl = 'http://alvinet.com/actualite/?keyword='.$keyword.
						'&akeyword='.$akeyword.
						'&catid='.$catid.
						'&periode=depuis&depuis='.$depuis.
						'&or='.$or.'';
	...
	function form($update){
        ...
        wp_parse_args($instance, $default);
